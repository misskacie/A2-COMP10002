...
main(int argc, char *argv[]) {
   ...
   // printf("%d %d %d %d", sizeof(automaton_t),sizeof(state_t),sizeof(list_t),sizeof(node_t));
...
void
print_stages(automaton_t *automaton, int *char_count, int *state_count, 
    int *freq_count) {
    //Stage 0
    printf(SDELIM, 0);
...
*string_concat(char *str1, char *str2) {
    ...
    char *rtr_str;
    rtr_str = (char*) malloc((strlen(str1)+strlen(str2) + 1));
    strcpy(rtr_str, str1);
    strcat(rtr_str, str2);
    free(str2);
    return rtr_str;
...
*insert_state(automaton_t *automaton, state_t *state, char *str) {
    char *newstr;
    newstr = create_string(strlen(str)+1);
    strcpy(newstr,str);
    node_t *current_node; 
    current_node = state->outputs->head;
